...
export class OptionsChainComponent {
  ...
  downloadCsv() {
    ...
    if (!d) {
      this.toast.error('There is no options chain data available to download.');
      return;
    }
    try {
      ...
      anchor.href = url;
      anchor.download = `${this.symbol}_option_chain.csv`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(url);
      this.toast.success(`${this.symbol} options chain exported.`);
    } catch {
      this.toast.error('Unable to download the options chain CSV. Please try again.');
    }
  }
}
